...
export const dishTypeEnum = zod.enum([
	"starter",
	"dish",
	"dessert",
]);

export const recipeFieldsSchema = zod.object({
	name: zod.string(),
...
const payloadRecipeFields = recipeFieldsSchema.pick({
	name: true,
	image: true,
	ingredients: true,
	personCount: true,
...
export const createRecipePayloadSchema = zod.object({
	records: zod.object({
		fields: payloadRecipeFields,
	}).array(),
});
export const updateRecipePayloadSchema = zod.object({
	records: zod.object({
		id: zod.string(),
...
export const recipeAirtableSchema = baseAirtableSchema.extend({
	fields: recipeFieldsSchema,
});
export type RecipeAirtable = ZodSpace.infer<typeof recipeAirtableSchema>;

export const recipeListAirtableSchema = zod.object({
